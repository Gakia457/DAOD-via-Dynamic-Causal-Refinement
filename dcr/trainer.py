"""Training loop orchestration for DCR, including EMA, distillation, and SP guidance hooks."""
...
from dcr.aug import WEAK_IMG_KEY, get_augs
from dcr.backbone import get_adamw_optim
from dcr.checkpoint import DetectionCheckpointerWithEMA
from dcr.distill import build_distiller
from dcr.dropin import DefaultTrainer, AMPTrainer, SimpleTrainer
from dcr.dataloader import SaveWeakDatasetMapper, UnlabeledDatasetMapper, WeakStrongDataloader
from dcr.dgcr import DiscrepancyGuider
from dcr.ema import EMA
from dcr.helpers import Detectron2COCOEvaluatorAdapter
from dcr.model import build_dcr
...
class _DCRTrainer:
     def __init__(self, model, data_loader, optimizer, distiller, backward_at_end=True, model_batch_size=None):
          ...
          self.trainer_ref = None  # DCR custom: call outer trainer hooks from inner trainer.

     def run_model(self, data):
          loss_dict = run_model_labeled_unlabeled(self, *data)

          if self.trainer_ref is not None and hasattr(self.trainer_ref, '_update_sp_params'):
               self.trainer_ref._update_sp_params(loss_dict)  # DCR custom: update SP guider each iteration.
...
class DCRAMPTrainer(_DCRTrainer, AMPTrainer): pass
class DCRSimpleTrainer(_DCRTrainer, SimpleTrainer): pass

class DCRTrainer(DefaultTrainer):
     """Modified DefaultTrainer to support Mean Teacher style training."""
     @staticmethod
     def _collect_distiller_optimizer_params(distiller):
          params = []
          if hasattr(distiller, "cross_attentions"):
               for module in distiller.cross_attentions.values():
     ...
     def _create_trainer(self, cfg, model, data_loader, optimizer):
          self.ema = EMA(build_dcr(cfg), cfg.EMA.ALPHA) if cfg.EMA.ENABLED else None
          self.distiller = build_distiller(cfg=cfg, teacher=self.ema.model if cfg.EMA.ENABLED else model, student=model)
          logger = logging.getLogger(__name__)

          if isinstance(self.distiller, torch.nn.Module):
               distiller_params = self._collect_distiller_optimizer_params(self.distiller)
               if distiller_params:
                    optimizer.add_param_group({'params': distiller_params})
                    logger.info("Added %d distiller parameters to optimizer.", len(distiller_params))
                    # print("Added %d distiller parameters to optimizer.", len(distiller_params))
                    logger.debug("Distiller parameter shapes: %s", [tuple(p.shape) for p in distiller_params])
                    # print("Distiller parameter shapes: %s", [tuple(p.shape) for p in distiller_params])

          self.sp_guider = DiscrepancyGuider(cfg)  # DCR custom: initialize SP discrepancy guider.
          trainer = (DCRAMPTrainer if cfg.SOLVER.AMP.ENABLED else DCRSimpleTrainer)(
            model, data_loader, optimizer, self.distiller,
            backward_at_end=cfg.SOLVER.BACKWARD_AT_END,
            model_batch_size=cfg.SOLVER.IMS_PER_GPU
          )
          trainer.trainer_ref = self  # DCR custom: expose outer trainer for SP updates.

          return trainer

     def _update_sp_params(self, loss_dict):
          """Delegate SP parameter updates to the discrepancy guider."""
          self.sp_guider.update_parameters(loss_dict, self.iter)  # DCR custom: SP update hook.
     
     def _create_checkpointer(self, model, cfg):
          checkpointer = super(DCRTrainer, self)._create_checkpointer(model, cfg,
          ...
          if self.distiller.distill_enabled() and getattr(self.distiller, 'do_feature_distill', False):
               checkpointer.add_checkpointable("distiller", self.distiller)
               logging.getLogger(__name__).info("Distiller registered to checkpointer.")
          return checkpointer
     ...
     def build_hooks(self):
          ret = super(DCRTrainer, self).build_hooks()
     ...
     def build_optimizer(cls, cfg, model):
          ...
          if cfg.SOLVER.OPTIMIZER is None or cfg.SOLVER.OPTIMIZER.upper() == "SGD":
               return super(DCRTrainer, cls).build_optimizer(cfg, model)
     ...
     def before_step(self):
          ...
          super(DCRTrainer, self).before_step()
          if self.cfg.EMA.ENABLED:
