"""
Augmentation utilities for weak/strong branches.
Implementation-wise, SP (Spectral Perturbation) is inserted at the start of strong augmentation.
This is logically equivalent to the paper pipeline. 
SP internals and dynamic parameter control are implemented in dgcr.py.
"""
...
from dcr.dgcr import SPTransform
...
def get_augs(cfg, labeled, include_strong_augs=True):
    ...
    if include_strong_augs:
        ...
        # Target domain: disable SPaug if SP_SOURCE_ONLY is enabled
        sp_prob = cfg.AUG.SP_PROB
        if cfg.AUG.SP_SOURCE_ONLY and not labeled:
            sp_prob = 0.0
        
        augs += build_strong_augmentation(
            include_erasing=include_random_erasing,
            sp_prob=sp_prob,
            sp_v1_scale=cfg.AUG.SP_V1_SCALE,
            sp_v2_scale=cfg.AUG.SP_V2_SCALE,
            sp_dynamic=cfg.AUG.SP_DYNAMIC_ADJUST
        )
...
def build_strong_augmentation(include_erasing=True, sp_prob=0.0, sp_v1_scale=0.005, sp_v2_scale=0.7, sp_dynamic=False):
    """
    Modified from Adaptive Teacher / Unbiased Teacher codebase
        ...
        - Add SP augmentation support
    Args:
        include_erasing: whether to include random erasing augmentation
        sp_prob: probability of applying SP transformation (0.0 = disabled)
        sp_v1_scale: SP v1 scale parameter for low-mid frequency
        sp_v2_scale: SP v2 scale parameter for mid-high frequency
        sp_dynamic: whether to use dynamic parameter adjustment
    """
    import logging
    logger = logging.getLogger(__name__)
    
    augs = []
    # Add SP augmentation if enabled
    if sp_prob > 0:
        augs.append(T.RandomApply(
            SPTransform(v1_scale=sp_v1_scale, v2_scale=sp_v2_scale, use_shared=sp_dynamic),
            prob=sp_prob
        ))
        mode = "dynamic" if sp_dynamic else "fixed"
        logger.info(
            "SP augmentation enabled (mode=%s, prob=%.2f, v1=%s, v2=%s)",
            mode,
            sp_prob,
            sp_v1_scale,
            sp_v2_scale,
