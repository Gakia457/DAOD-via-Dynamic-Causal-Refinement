"""Checkpointer that can start from EMA weights and restore an optional distiller state."""
...
class DetectionCheckpointerWithEMA(DetectionCheckpointer):
    ...
    def resume_or_load(self, path: str, *, resume: bool = True) -> Dict[str, Any]:
        ...
        if "distiller" in ret.keys() and "distiller" in self.checkpointables:
            self.checkpointables["distiller"].load_state_dict(ret["distiller"])
            self.logger.info("Loaded distiller parameters: %d", len(ret["distiller"]))
